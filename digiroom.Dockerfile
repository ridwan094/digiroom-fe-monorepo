...
RUN turbo prune --scope=digiroom --docker
...
COPY apps/digiroom/package.json /app/apps/digiroom/package.json
...
RUN turbo run build --no-cache --filter=digiroom
...
CMD ["yarn", "turbo", "run", "dev", "--filter=digiroom", "--no-daemon"]