...
RUN turbo prune --scope=atomic-design --docker
...
COPY apps/boilerplate/package.json /app/apps/boilerplate/package.json
...
RUN turbo run build --no-cache --filter=atomic-design
...
CMD ["yarn", "turbo", "run", "dev", "--filter=atomic-design", "--no-daemon"]