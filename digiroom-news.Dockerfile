...
RUN turbo prune --scope=digiroom-article --docker

#############################################
FROM base AS installer
WORKDIR /app
ARG digiroom-article
...
COPY apps/digiroom-news/package.json /app/apps/digiroom-news/package.json
...
RUN turbo run build --no-cache --filter=digiroom-articles
...
CMD ["yarn", "turbo", "run", "dev", "--filter=digiroom-article", "--no-daemon"]