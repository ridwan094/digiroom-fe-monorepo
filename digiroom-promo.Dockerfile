...
RUN turbo prune --scope=digiroom-promo --docker
...
COPY apps/digiroom-promo/package.json /app/apps/digiroom-promo/package.json
...
RUN turbo run build --no-cache --filter=digiroom-promo
...
CMD ["yarn", "turbo", "run", "dev", "--filter=digiroom-promo", "--no-daemon"]