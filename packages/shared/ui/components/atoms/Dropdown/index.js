      ...
      {withInput ? (
        <Fragment>
          ...
          <div className="flex items-center">
            ...
            {/* Handle toggle icon */}
            {selectedOption ? (
              <span className="absolute right-2 text-red-500 cursor-pointer" onClick={deleteSelectedOption}>
                <FiX />
              </span>
            ) : (
              <span
                ...
                onClick={toggleDropdown}
              >
                <svg width="11" height="8" viewBox="0 0 10 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
        ...
        <div className="flex gap-1 items-center cursor-pointer h-10 justify-center px-2" onClick={toggleDropdown}>
          {icon ? icon : <div>{text}</div>}

          <MdArrowDropDown style={{ color: "#494949", rotate: isDropdownOpen ? "180deg" : "0deg" }} />
        </div>
      )}
      {isDropdownOpen && (
